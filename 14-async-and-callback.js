...
function testLogicPromise() {
    ...
    .then(value => {    // Promise 내부에서 resolve 함수로 전달해준 값(fullfill 상태로)
        console.log("PROMISE resolve: ", value);
    })
    .catch(reason => {  // Promise 내부에서 reject 함수로 전달해준 값 (rejected 상태로)
...
async function processData() {
    ...
    try {
        // await 키워드 : Promise가 해결될 때까지 코드를 일시중지 시킴
        //                Promise 해소되면 Promise의 결과값을 반환
        // await 키워드 함수 사용을 위해 async가 붙어 있어야 함
